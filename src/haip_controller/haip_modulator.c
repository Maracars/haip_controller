...
int haip_modulate_frame(unsigned char* frame_buffer, int frame_length, fract32* modulated_signal) {
	...
	oscilate(modulated_signal, frame_symbols*HAIP_CODING_RATE + HAIP_PREAMBLE_SYMBOLS);
...
void filter(int length) {
	...
	for (i = length * HAIP_OVERSAMPLING_FACTOR; i< length * HAIP_OVERSAMPLING_FACTOR + HAIP_SRCOS_COEFF_NUM; i++){
		frame_symbols_real_upsample[i] = 0;
		frame_symbols_imag_upsample[i] = 0;
	}
	...
	//Filters the signal
	fir_fr32(frame_symbols_real_upsample, filtered_real_symbols, length * HAIP_OVERSAMPLING_FACTOR + HAIP_SRCOS_COEFF_NUM, &state_real);
	fir_fr32(frame_symbols_imag_upsample, filtered_imag_symbols, length * HAIP_OVERSAMPLING_FACTOR + HAIP_SRCOS_COEFF_NUM, &state_imag);
...
void oscilate(fract32* modulated_signal, int length) {
	...
	for (i = 0; i < length * HAIP_OVERSAMPLING_FACTOR + HAIP_SRCOS_COEFF_NUM; ++i) {
		modulated_signal[i] = 	(filtered_real_symbols[i + HAIP_SRCOS_FILTER_DELAY] * cos_modulator_6KHz[i % HAIP_OVERSAMPLING_FACTOR] -
								filtered_imag_symbols[i + HAIP_SRCOS_FILTER_DELAY] * sin_modulator_6KHz[i % HAIP_OVERSAMPLING_FACTOR]) * SQRT_2;
	}
}
